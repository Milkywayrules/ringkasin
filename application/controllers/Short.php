<?php
  ...
  class Short extends CI_Controller
  {
    ...
    // method untuk membuat data shortened link berdasarkan link panjang yang dimasukkan oleh user
    {
      ...
      // $this->form_validation->set_rules('custom', 'custom URL', 'trim|required|is_unique[url.custom_url]');
      ...
      if ( $this->session->createCustom == '_rngksn_cstm_xx_' ) {
      ...
      // $exist = $this->cek_custom($createUrl); // TIDAK DIPAKAI UNTUK SEMENTARA | UNTUK BUAT CEK CUSTOM NAME PADA DB
      $exist = FALSE;
      ...
      if ($exist == FALSE) {
        ...
        if ($this->session->username == '') {
          ...
          if ($this->M_url->create_url($createUrl, $short, $createCustom, $this->session->qrcode_img, 'Guest')) {
            ...
            echo "Gagal create data. Hubungi kami jika masalah masih berlanjut :) - Kode error : s/c/1";
          ...
          if ( $this->M_url->create_url($createUrl, $short, $createCustom, $this->session->qrcode_img, $this->session->username) ) {
            ...
            // jika gagal create data pada model create_url()
            echo "Gagal create data. Hubungi kami jika masalah masih berlanjut :) - Kode error : s/c/2";
            header("Refresh: 5; URL=".base_url());
          }
        }
      }else {
